...
import { MINUTE } from "./timeParser";

export async function HandleAnchor(client: Client, message: Message) {
    ...
    try {
      if (anchor.lastAnchorId) {
        try {
          ...
          console.error("Failed to delete the old anchor message:", err);
        }
      }
      ...
      console.error("Error during reanchoring process:", err);
    }
  }
}

export async function CheckAnchorInactivity(client: Client) {
  const now = Date.now();
  ...
  if (!anchors || anchors.length === 0) {
    setTimeout(() => CheckAnchorInactivity(client), MINUTE * 5);
    return;
  }
    ...
    try {
      ...
      if (anchor.lastAnchorId) {
        try {
          ...
          console.error("Error deleting old anchor on inactivity check:", err);
        }
      }
      ...
      console.error(`Error processing inactivity for anchor ${anchor._id}:`, err);
    }
  }
  ...
  if (nextAnchor) {
    ...
    setTimeout(() => CheckAnchorInactivity(client), nextCheck);
  } else {
    setTimeout(() => CheckAnchorInactivity(client), MINUTE * 5);
  }
}
